...
function randomBuildData(seed) {
    ...
    for (var i = 1; i < 92; i++) {
        ...
        returnData[datStr] = Math.ceil(Math.random() * seed);  //设置每一天的数据
        dat.setDate(dat.getDate() + 1);  //通过set时间发生了变化
    }
    return returnData;
}
//为了渲染选择器
...
function initAqiChartData() {
    ...
    for (var city in aqiSourceData) { //city是城市
        ...
        week[city] = {};
        month[city] = {};
        for (var date in aqiSourceData[city]) { //再次遍历，把每个城市对应的数据挑出来,date是每一天的日期
